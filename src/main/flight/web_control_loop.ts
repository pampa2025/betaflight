...
export function startWebControlLoop(opts: WebControlLoopOptions): WebControlLoopHandle {
  ...
  const cfg = (isFullCfg(opts.cfg) ? opts.cfg as PidMinConfig : normalizePidMinConfigForWeb(opts.cfg as Partial<PidMinConfig>));
  ...
  // Fixed-step scheduler with drift compensation
  ...
  const tick = () => {
    ...
    // Run one or more control steps to catch up if delayed
