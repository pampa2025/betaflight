...
import { normalizePidMinConfigForWeb, normalizePidMinLaunchConfig, PidMinLaunchMode, Axis3 } from '../../pid_min';
import { pidMinWebFeedforwardUpdateAll, normalizePidMinWebFeedforwardRuntime, PidMinWebFeedforwardState, makePidMinWebFeedforwardMultiState } from '../../pid_min_feedforward_web';

type TeleEl = { outRoll: HTMLElement; outPitch: HTMLElement; outYaw: HTMLElement; gyroRoll: HTMLElement; gyroPitch: HTMLElement; gyroYaw: HTMLElement; joyRoll: HTMLElement; joyPitch: HTMLElement; joyYaw: HTMLElement; pitchAngle: HTMLElement; startBtn: HTMLButtonElement; stopBtn: HTMLButtonElement; rateInput: HTMLInputElement; };
...
const targetBufs: Axis3<number[]> = { roll: new Array(plotBufSize).fill(0), pitch: new Array(plotBufSize).fill(0), yaw: new Array(plotBufSize).fill(0) };
const gyroBufs: Axis3<number[]> = { roll: new Array(plotBufSize).fill(0), pitch: new Array(plotBufSize).fill(0), yaw: new Array(plotBufSize).fill(0) };
...
function renderPlotAxis(axis: AxisKey) {
  ...
  ctx.fillStyle = '#9ea0a8'; ctx.font = '12px system-ui';
  ctx.fillText(`±${plotYRange} deg/s`, 8, 14);
  ctx.fillText('target: teal, gyro: orange', w - 200, 14);
  const count = plotCounts[axis]; if (count === 0) return;
  const toY = (v: number) => Math.round(h / 2 - (v / plotYRange) * (h / 2 - 8));
  const stepX = w / (plotBufSize - 1);
  const tb = targetBufs[axis]; const gb = gyroBufs[axis]; const baseIdx = plotIdxs[axis];
  ctx.strokeStyle = '#11c2b3'; ctx.lineWidth = 2; ctx.beginPath();
  for (let i = 0; i < plotBufSize; i++) {
    const idx = (baseIdx + i) % plotBufSize; const x = i * stepX; const y = toY(tb[idx]);
    if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
  }
  ctx.stroke();
  ctx.strokeStyle = '#ff8a3d'; ctx.lineWidth = 2; ctx.beginPath();
  for (let i = 0; i < plotBufSize; i++) {
    const idx = (baseIdx + i) % plotBufSize; const x = i * stepX; const y = toY(gb[idx]);
    if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
  }
  ctx.stroke();
}
...
function startLoop() {
  const controlRateHz = Math.max(60, Math.min(1000, Number(el.rateInput.value) || 300));
...
function tickUi() { renderTelemetry(); requestAnimationFrame(tickUi); }
requestAnimationFrame(tickUi);

// Render plots at RAF cadence
function tickPlot() { renderPlotAxis('roll'); renderPlotAxis('pitch'); renderPlotAxis('yaw'); requestAnimationFrame(tickPlot); }
