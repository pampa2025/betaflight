...
export function normalizeLoggerOptions(
	inOpts?: Partial<GamepadLoggerOptions>
): GamepadLoggerOptions {
	return {
...
export function getConnectedGamepads(indexFilter?: number[]): Gamepad[] {
	...
	if (!list) return [];
	const pads = Array.from(list).filter(
		(gp): gp is Gamepad => !!gp && gp.connected
	);
	if (indexFilter && indexFilter.length > 0)
		return pads.filter((gp) => indexFilter.includes(gp.index));
	return pads;
}
...
export function formatGamepadSummary(
	gp: Gamepad,
	opts?: { logAxes?: boolean; logButtons?: boolean }
): string {
	const base = formatGamepadHeader(gp);
	const parts: string[] = [base];
	if (opts?.logAxes) {
		const axesStr = gp.axes
			.map((a) => (Number.isFinite(a) ? a.toFixed(2) : '0.00'))
			.join(',');
...
export function startGamepadLogger(inOpts?: Partial<GamepadLoggerOptions>): {
	stop: () => void;
} {
	...
	if (opts.initialScan) {
		...
		if (pads.length === 0) {
			...
			for (const gp of pads)
				console.info(`[gamepad] detected ${formatGamepadHeader(gp)}`);
		}
	}
	...
	if (opts.pollIntervalMs > 0) {
		timer = setInterval(() => {
			...
			for (const gp of pads) {
				console.info(
					formatGamepadSummary(gp, {
						logAxes: opts.logAxes,
						logButtons: opts.logButtons,
					})
				);
			}
