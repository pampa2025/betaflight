...
export function normalizeGamepadReadConfig(
	inCfg?: Partial<GamepadReadConfig>
): GamepadReadConfig {
	return {
		index: inCfg?.index,
...
export function normalizeGamepadThrottleConfig(
	inCfg?: Partial<GamepadThrottleConfig>
): GamepadThrottleConfig {
	return {
		index: inCfg?.index,
		axis: inCfg?.axis ?? 1,
...
export function readGamepadAxes(
	inCfg?: Partial<GamepadReadConfig>,
	padOrIndex?: Gamepad | number
): Axis3<number> {
...
export function readGamepadThrottle(
	inCfg?: Partial<GamepadThrottleConfig>,
	padOrIndex?: Gamepad | number
): number {
...
function resolveGamepad(padOrIndex?: Gamepad | number): Gamepad | null {
	try {
		...
		const list: (Gamepad | null)[] | null =
			nav && typeof nav.getGamepads === 'function' ? nav.getGamepads() : null;
		if (!list) return null;
