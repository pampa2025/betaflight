...
function makeStepPitchScenario(
	stepTimeSec: number,
	magnitude: number
): Scenario {
	return {
		...
		joystickAt: (t: number) => ({
			roll: 0,
			pitch: t >= stepTimeSec ? magnitude : 0,
			yaw: 0,
		}),
	};
}
...
function makeSineYawScenario(freqHz: number, amplitude: number): Scenario {
	return {
		...
		joystickAt: (t: number) => ({
			roll: 0,
			pitch: 0,
			yaw: amplitude * Math.sin(2 * Math.PI * freqHz * t),
		}),
	};
}
...
export function runPidWebTest(opts?: {
	...
	webRtOverrides?: Partial<
		Parameters<typeof normalizePidMinWebFeedforwardRuntime>[0]
	>;
	scenario?: Scenario;
	...
	const coeffs: PidMinCoefficients = opts?.coeffs ?? {
		Kp: 0.9,
		Ki: 0.3,
		Kd: 0.02,
		Kf: 0,
	...
	const logEveryN = Math.max(
		1,
		opts?.logEveryN ?? Math.floor(controlRateHz / 10)
	); // ~6 Hz logs
	...
	if (!useCsv) {
		console.info(
			`[pid-web-test] scenario=${scenario.name} rate=${controlRateHz}Hz duration=${durationSec}s`
		);
	} else if (opts?.csvHeaders !== false) {
		// Header: t,joy_*,target_*,gyro_*,out_*
		console.log(
			't,joy_roll,joy_pitch,joy_yaw,target_roll,target_pitch,target_yaw,gyro_roll,gyro_pitch,gyro_yaw,out_roll,out_pitch,out_yaw'
		);
	}
		...
		if (useCsv) {
			if (i % csvEveryN === 0) {
				console.log(
					[
						t.toFixed(decimals),
						joystick.roll.toFixed(decimals),
						joystick.pitch.toFixed(decimals),
						joystick.yaw.toFixed(decimals),
						ffRes.targetRatesDegS.roll.toFixed(decimals),
						ffRes.targetRatesDegS.pitch.toFixed(decimals),
						ffRes.targetRatesDegS.yaw.toFixed(decimals),
						gyro.roll.toFixed(decimals),
						gyro.pitch.toFixed(decimals),
						gyro.yaw.toFixed(decimals),
						outputs.roll.toFixed(decimals),
						outputs.pitch.toFixed(decimals),
						outputs.yaw.toFixed(decimals),
					].join(',')
				);
			}
		} else if (i % logEveryN === 0) {
			console.info(
				`[t=${t.toFixed(2)}] joy={${joystick.roll.toFixed(
					2
				)}, ${joystick.pitch.toFixed(2)}, ${joystick.yaw.toFixed(2)}} ` +
					`gyro={${gyro.roll.toFixed(2)}, ${gyro.pitch.toFixed(
						2
					)}, ${gyro.yaw.toFixed(2)}} ` +
					`out={${outputs.roll.toFixed(2)}, ${outputs.pitch.toFixed(
						2
					)}, ${outputs.yaw.toFixed(2)}}`
			);
		}
	}
